...
def get_components(line_arpas, arpa_words):
    ...
    vowels = [Vowel(i, line_arpas[i], vowel_color_dict[re.sub(r'\d+', '', line_arpas[i])][1],
              vowel_color_dict[re.sub(r'\d+', '',
                                      line_arpas[i])][0], arpa_words[i], int(re.sub('[^0-9]', '', line_arpas[i])))
              for i in range(len(line_arpas)) if contain_nums(line_arpas[i])]
    
    consonants = [Consonant(i, line_arpas[i], consonant_color_dict[re.sub(r'\d+', '', line_arpas[i])][1],
